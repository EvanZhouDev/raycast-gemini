import {
    List, ActionPanel, Action, getPreferenceValues, Toast, Icon,
    showToast, Form, useNavigation, confirmAlert
} from "@raycast/api";
...
export default function Chat() {
    ...
    let CreateChat = () => {
        ...
        return (
            <Form
                actions={
                    <ActionPanel>
                        <Action.SubmitForm title="Create Chat" onSubmit={(values) => {
                            if (values.chatName === "") {
    ...
    let GeminiActionPanel = () => {
        return <ActionPanel>
            <Action icon={Icon.Message} title="Send to Gemini" onAction={() => {
                if (searchText === "") {
                ...
                setSearchText("");
                if (getChat(chatData.currentChat).messages.length == 0 || getChat(chatData.currentChat).messages[0].finished) {
            ...
            <ActionPanel.Section title="Manage Chats">
                <Action.Push icon={Icon.PlusCircle} title="Create Chat" target={<CreateChat />} shortcut={{ modifiers: ["cmd"], key: "n" }} />
                <Action icon={Icon.ArrowDown} title="Next Chat" onAction={() => {
                    let chatIdx = 0;
                ...
                <Action icon={Icon.ArrowUp} title="Previous Chat" onAction={() => {
                    let chatIdx = 0;
                    for (let i = 0; i < chatData.chats.length; i++) {
                        if (chatData.chats[i].name === chatData.currentChat) {
                            chatIdx = i;
            ...
            <ActionPanel.Section title="Danger zone">
                <Action icon={Icon.Trash} title="Delete Chat" onAction={async () => {
                    await confirmAlert({
                        title: "Are you sure?", message: "You cannot recover this chat.", icon: Icon.Trash, primaryAction: {
                ...
                }} shortcut={{ modifiers: ["cmd", "shift"], key: "delete" }} style={Action.Style.Destructive} />
            </ActionPanel.Section>
        </ActionPanel>
    }
    ...
    return (
        chatData === null ? <List searchText={searchText}
